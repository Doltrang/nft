...
//for Dmail
impl Ledger {
    pub fn query_nft_by_name(&self, alias: String) -> Option<Principal> {
    ...
    pub fn query_bind_nft_by_pid(&mut self, pid: Principal) -> Option<TokenMetadata> {
        for token_metadata_item in ledger().tokens.values() {
            if token_metadata_item.principal == pid
                && token_metadata_item.metadata_desc[0].key_val_data[1].val
                    == MetadataVal::TextContent("true".to_string())
            {
                return Some(token_metadata_item.clone());
            }
        }
        None
    }

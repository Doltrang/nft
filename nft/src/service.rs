...
async fn mint_dip721(to: Principal, metadata_desc: MetadataDesc) -> MintReceipt {
    ...
    if let MetadataVal::TextContent(alias) = metadata_desc[0].key_val_data[0].val.clone() {
        ...
        metadata_desc[0].key_val_data[0].val = MetadataVal::TextContent(normalize_alias);
    ...
    //         (
    //             "alias".into(),
    //             DetailValue::Text(metadata_desc[0].key_val_data[0].key.clone()),
    //         ),
    //     ])
    //     .build()
    //     .unwrap();
    // let tx_id = insert_into_cap(event).await?;
